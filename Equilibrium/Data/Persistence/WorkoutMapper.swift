...
enum WorkoutMapper {
    static func record(from workout: ScheduledWorkout) -> ScheduledWorkoutRecord {
        let exercises = workout.exercises.enumerated().map { exerciseIndex, exercise in
            ...
            return ScheduledExerciseRecord(id: exercise.id.rawValue, exerciseID: exercise.exerciseID.rawValue, nameSnapshot: exercise.nameSnapshot, position: exerciseIndex, restDuration: exercise.restDuration, skippedAt: exercise.skippedAt, prescriptions: prescriptions, loggedSets: logged)
        }
        return ScheduledWorkoutRecord(id: workout.id.rawValue, localDay: workout.day.iso8601, titleSnapshot: workout.titleSnapshot, templateID: workout.templateID?.rawValue, planID: workout.planID?.rawValue, sourceRaw: workout.source.rawValue, statusRaw: workout.status.rawValue, startedAt: workout.startedAt, completedAt: workout.completedAt, createdAt: workout.createdAt, updatedAt: workout.updatedAt, exercises: exercises)
    }

    static func domain(from record: ScheduledWorkoutRecord) throws -> ScheduledWorkout {
        let exercises = try record.exercises.sorted { $0.position < $1.position }.map { record in
            ...
            return ScheduledExercise(id: ScheduledExerciseID(rawValue: record.id), exerciseID: ExerciseID(rawValue: record.exerciseID), nameSnapshot: record.nameSnapshot, prescriptions: prescriptions, loggedSets: sets, restDuration: record.restDuration, skippedAt: record.skippedAt)
        }
        guard let source = WorkoutSource(rawValue: record.sourceRaw), let status = WorkoutStatus(rawValue: record.statusRaw) else { throw RepositoryError.invalidBackup }
        return ScheduledWorkout(id: ScheduledWorkoutID(rawValue: record.id), day: try LocalDay(record.localDay), titleSnapshot: record.titleSnapshot, templateID: record.templateID.map(WorkoutTemplateID.init(rawValue:)), planID: record.planID.map(PlanID.init(rawValue:)), source: source, exercises: exercises, status: status, startedAt: record.startedAt, completedAt: record.completedAt, createdAt: record.createdAt, updatedAt: record.updatedAt)
    }
}
