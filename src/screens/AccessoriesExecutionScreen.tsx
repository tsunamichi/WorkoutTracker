...
export function AccessoriesExecutionScreen() {
  ...
  const accessoryItems = migrateItemsArray(rawAccessoryItems as any);
  ...
  const allGroupsComplete = useMemo(() => {
    if (accessoryGroups.length === 0) return false;
    return accessoryGroups.every(group => {
      const rounds = currentRounds[group.id] || 0;
      return rounds >= group.totalRounds;
    });
  }, [accessoryGroups, currentRounds]);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={styles.itemsAccordion}>
          {accessoryGroups.map((group, index) => {
            ...
            const groupCardBg = isCompleted ? styles.itemCardDimmed : (isExpanded ? styles.itemCard : styles.itemCardInactive);
            const groupCardFg = isCompleted ? styles.itemCardInnerDimmed : (isExpanded ? styles.itemCardInner : styles.itemCardInnerInactive);
                        ...
                        return (
                          <React.Fragment key={exercise.id}>
                            ...
                            <TouchableOpacity
                              ...
                              disabled={false}
                              testID={`exercise-container-${exIndex}`}
                            >
                              <View style={[
                                ...
                                {/* Values Row - Only show for active exercise */}
                                {isActive && (
      ...
      {expandedGroupIndex >= 0 && accessoryGroups[expandedGroupIndex] && (() => {
        const activeEx = accessoryGroups[expandedGroupIndex].exercises[activeExerciseIndex];
      ...
      >
        <View style={styles.adjustmentDrawerContent}>
          <Text style={styles.adjustmentDrawerTitle} numberOfLines={2}>
            {expandedGroupIndex >= 0 && accessoryGroups[expandedGroupIndex]?.exercises[activeExerciseIndex]?.movementId || t('adjustValues')}
          </Text>
