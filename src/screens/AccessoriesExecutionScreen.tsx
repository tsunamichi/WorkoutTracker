...
type RouteParams = {
  AccessoriesExecution: {
    workoutKey: string;
    workoutTemplateId: string;
  };
};

export function AccessoriesExecutionScreen() {
  const navigation = useNavigation();
  const route = useRoute<RouteProp<RouteParams, 'AccessoriesExecution'>>();
  const insets = useSafeAreaInsets();
  const { t } = useTranslation();
  
  const { workoutKey, workoutTemplateId } = route.params;
  const { getWorkoutTemplate, updateAccessoryCompletion, getAccessoryCompletion, updateWorkoutTemplate, settings, addAccessoryToSession } = useStore();
  ...
  // Group accessory items into groups (supersets or single exercises)
  const accessoryGroups = (() => {
    const result: Array<{
      ...
      cycleId?: string;
      totalRounds: number;
      exercises: Array<typeof accessoryItems[0]>;
    }> = [];
    
    const cycleGroups: Record<string, typeof accessoryItems> = {};
  ...
  useEffect(() => {
    ...
    const firstIncompleteIndex = accessoryGroups.findIndex(group => {
      const round = rounds[group.id] || 0;
      return round < group.totalRounds;
    });
    if (firstIncompleteIndex !== -1) {
      setExpandedGroupIndex(firstIncompleteIndex);
      const group = accessoryGroups[firstIncompleteIndex];
  ...
  useEffect(() => {
    if (expandedGroupIndex >= 0 && accessoryGroups[expandedGroupIndex]) {
      const group = accessoryGroups[expandedGroupIndex];
  ...
  const saveAccessorySession = async () => {
    // Collect all completed accessory sets
    const accessorySets: import('../types').AccessorySet[] = [];
  ...
  const handleStart = () => {
    ...
    const currentGroup = accessoryGroups[expandedGroupIndex];
      ...
      if (allExercisesCompleted) {
        ...
        if (currentRound + 1 >= currentGroup.totalRounds) {
          ...
          if (nextIncompleteIndex !== -1) {
            ...
            // All done, save accessory session and navigate back
            await saveAccessorySession();
            navigation.goBack();
          }
        } else {
  ...
  const allComplete = accessoryGroups.length > 0 && accessoryGroups.every(group => 
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
            // Update the template to remove accessory items
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        <View style={styles.itemsAccordion}>
          {accessoryGroups.map((group, index) => {
      ...
      {expandedGroupIndex >= 0 && accessoryGroups[expandedGroupIndex] && (
        <SetTimerSheet
          ...
          exerciseName={accessoryGroups[expandedGroupIndex].exercises[activeExerciseIndex]?.exerciseName}
          currentSet={currentRounds[accessoryGroups[expandedGroupIndex].id] + 1}
          totalSets={accessoryGroups[expandedGroupIndex].totalRounds}
          isExerciseTimerPhase={isExerciseTimerPhase}
          exerciseDuration={localValues[accessoryGroups[expandedGroupIndex].exercises[activeExerciseIndex]?.id]?.reps ?? accessoryGroups[expandedGroupIndex].exercises[activeExerciseIndex]?.reps ?? 30}
          onExerciseTimerComplete={handleComplete}
          skipRestPhase={true}
          isPerSide={accessoryGroups[expandedGroupIndex].exercises[activeExerciseIndex]?.isPerSide}
        />
      )}
