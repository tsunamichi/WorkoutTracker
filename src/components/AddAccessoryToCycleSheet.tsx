...
interface AddAccessoryToCycleSheetProps {
  visible: boolean;
  onClose: () => void;
  onAdd: (newItem: Omit<AccessoryItem, 'id'>) => void;
  cycleSets: number; // Number of sets that all items in the cycle must have
}

export const AddAccessoryToCycleSheet = ({
  visible,
  onClose,
  onAdd,
  cycleSets,
}: AddAccessoryToCycleSheetProps) => {
  ...
  const handleAdd = () => {
    ...
    const newItem: Omit<AccessoryItem, 'id'> = {
